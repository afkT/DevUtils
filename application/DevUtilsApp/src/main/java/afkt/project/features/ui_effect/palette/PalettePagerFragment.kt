...
import afkt.project.databinding.FragmentUiEffectPalettePagerBinding
...
// =====================================
// = Palette 效果展示 ViewPager Fragment =
// =====================================

fun newPalettePagerFragment(position: Int) = PalettePagerFragment().apply {
    arguments = bundleOf(DevFinal.STR.POSITION to position)
}

class PalettePagerFragment : AppFragment<FragmentUiEffectPalettePagerBinding, PaletteViewModel>(
    R.layout.fragment_ui_effect_palette_pager, vmType = FragmentVMType.APPLICATION
) {
    ...
    // 不需要设置顶部状态栏、底部导航栏边距
    override fun isApplyWindowInsets(): Boolean = false
