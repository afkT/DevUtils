...
/**
 * 根据双方比对值接口实现是否在接口语义下相等控制选中状态。
 * <pre>
 *     对应布局属性 binding_selected_IfCompareValueEquals 与 binding_selected_IfCompareValueEquals_value2；
 *     相等为选中，否则未选中；内部委托扩展 `compareValueEquals` 与同文件 [bindingSelected]；
 ...
 */
@BindingAdapter(
    "binding_selected_IfCompareValueEquals",
    "binding_selected_IfCompareValueEquals_value2",
)
fun View.bindingSelectedIfCompareValueEquals(
    value1: CompareValue?,
    value2: CompareValue?
) {
    bindingSelected(value1.compareValueEquals(value2))
}