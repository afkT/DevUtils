...
description: >-
  ...
  同步 `{DEPS_ROOT}/versions.gradle` 中集中版本号，并为缺失的开源地址补全注释。在用户要求
  升级依赖、检查过时库版本、对齐 config/deps 与中央仓库或 GitHub 发布时使用。
...
| 符号 | 当前值（DevUtils） | 说明 |
|------|-------------------|------|
| `DEPS_ROOT` | `file/gradle` | 坐标定义、`versions.gradle` 与 `config_*.gradle` |
| `DEPS_MANIFEST` | `file/deps` | 业务侧 deps 清单（仅分工说明中引用） |

## 适用范围

| 路径 | 是否纳入 |
|------|----------|
| `{DEPS_ROOT}/config.gradle`（`ext.deps`） | 是：含 `build` 插件、官方栈、Dev 系列等 |
...
3. **同步写回**：确认目标版本后，**同一轮**更新 `{DEPS_ROOT}/config.gradle` / `{DEPS_ROOT}/config_libs.gradle`（及受影响的 `{DEPS_ROOT}/config_*.gradle`）与 **`{DEPS_ROOT}/versions.gradle`** 中被引用的变量；同一库多行（如 `glide` + `glide_compiler`）保持版本一致。
...
### C. Gradle 插件（`{DEPS_ROOT}/config.gradle` 的 `build` 等）
...
- **Kotlin / Android Gradle Plugin / 编译器**：与 `{DEPS_ROOT}/versions.gradle`、Android Studio 兼容矩阵一致；不单为「数字更新」而升到未验证组合。
...
| `${versions.xxx}` | 改 **`{DEPS_ROOT}/versions.gradle`** 中对应 `xxx`，**不要**在 `{DEPS_ROOT}/config*.gradle` 里硬编码拆变量 |
...
- [ ] 交叉校验；记录最终采用版本与依据链接
- [ ] 更新 `{DEPS_ROOT}/config.gradle` / `{DEPS_ROOT}/config_libs.gradle` / `{DEPS_ROOT}/config_*.gradle` 与 `{DEPS_ROOT}/versions.gradle`（若涉及）
- [ ] 补全缺失的开源/文档注释链接
