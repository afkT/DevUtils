...
description: >-
  ...
  R.styleable.DevWidget 中相关 app:dev_* 属性及行为。工作区内优先 Read lib/DevWidget；
  仅当无该模块时再查上游 GitHub。用于布局 XML、圆角卡片、背景、头像圆圈、减少 drawable。
---
...
**上游 fallback**（仅工作区无 `{DEVWIDGET_ROOT}` 或需对照远程 tag 时一行查阅）：https://github.com/afkT/DevUtils/tree/master/lib/DevWidget
...
**选控件**：与原生同名布局一致即可（要 Constraint 用 `RoundConstraintLayout`，要文字用 `RoundTextView`…）。完整类列表以 `{DEVWIDGET_ROOT}/src/main/java/dev/widget/ui/round/` 为准。

**RoundImageView 与其它 Round* 的差异**：仅负责**圆形**头像式展示（`CENTER_CROP`）；若要圆角矩形图片请不在本 Skill 范围内另选方案。

## XML 属性（`declare-styleable` 名：`DevWidget`）

属性定义以 **`{DEVWIDGET_ROOT}/src/main/res/values/attrs.xml`** 为准；所有属性在资源中声明为 **`DevWidget`** 分组，XML 前缀一般为 **`app:`**。

### A. RoundDrawable 系（`RoundFrameLayout` / `RoundLinearLayout` / `RoundConstraintLayout` / `RoundTextView` 等）
...
**圆角逻辑摘要**（对照 `{DEVWIDGET_ROOT}/.../round/RoundDrawable.java` 中 `fromAttributeSet`）：
...
- 链式 API：`setBgData`、`setStrokeData` / `setStrokeColors`、`setRadiusAdjustBounds`、`getStrokeWidth` 等 → Read `{DEVWIDGET_ROOT}/src/main/java/dev/widget/ui/round/RoundDrawable.java`。
